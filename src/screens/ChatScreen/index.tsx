...
import { StyleSheet, TextInput, TouchableOpacity, KeyboardAvoidingView, FlatList, StatusBar } from 'react-native'
...
const ChatScreen = () => {
    ...
    const renderMessage = ({ item: message, index }: { item: MessageComponentDataType, index: number }) => {
        const prevIndex = (index - 1) < 0 ? 0 : (index - 1);
        return <Message message={message} isSame={index === 0 ? false : (message.type === DATA[prevIndex].type)} />
    }

    const renderChatContent = () => {
        return <View flex paddingB-70 bottom>
            <FlatList
    ...
    return (
        <View flex backgroundColor={Colors.white}>
            ...
            <NavBar title={sender} leftIcon='back' onLeftClick={onGoBack} rightIcon={Assets.icons.ic_post_menu_black} />
