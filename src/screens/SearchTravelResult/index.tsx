...
const SearchTravelResultScreen = () => {
  ...
  const [isOpenFullPeopleView, setIsOpenFullPeopleView] =
          ...
          <View row centerV>
            <View flex-2 row>
              <View row centerV>
                ...
                <Text c83 M13 marginL-10>
                  {item.rate}{' '}
                </Text>
              </View>
              <View row centerV marginL-medium>
                <Image
                  source={Assets.icons.ic_article_comment}
                  width={14}
                  height={14}
                />
                <Text c83 marginL-10>
        ...
        <View
          ...
          <TouchableOpacity
            ...
            <View row centerV right paddingR-small marginT-medium>
              <Text marginR-10 B13 c04 uppercase>
                Show All
              </Text>
  ...
  const renderPeopleBeenThere = useCallback(
    () => (
      <View paddingV-32 backgroundColor={Colors.white}>
        ...
        {DATA_5.slice(0, 5).map((item, idx) => (
          <FollowItem key={idx} {...item} isFollowing />
        ))}
  ...
  return (
    <View flex backgroundColor={Colors.cf7}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View flex>
          {renderSection(
            DATA,
            'Trips for New York',
            'Stories curated to your interests',
            'trip',
          )}
          {renderSection(
            DATA_2,
      ...
      {isOpenFullPeopleView && (
        <ListPeopleView
          data={DATA_5}
          onClose={() => setIsOpenFullPeopleView(false)}
        />
      )}
      {isOpenTripView && (
        <ListTripView
          title="Trips for New York"
          data={DATA_6}
          onClose={() => setIsOpenTripView(false)}
        />
      )}
      {isOpenPlaceView && (
        <ListPlaceView
          data={DATA_2}
          onClose={() => setIsOpenPlaceView(false)}
        />
      )}
    </View>
  );
