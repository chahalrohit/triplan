...
import { View, Text, Picker, Assets, Colors, Image, KeyboardAwareScrollView } from 'react-native-ui-lib'
import { useNavigation } from '@react-navigation/native'
import NavBar from 'components/NavBar'
import { width } from 'config/scaleAccordingToDevice'
import { FONTS } from 'config/FoundationConfig'
import { ICChange } from '../../images'
                ...
                <View flex paddingT-medium backgroundColor={Colors.white}>
                    <View row centerV spread paddingV-small paddingH-medium>
                        <Picker
                            useNativePicker
                            value={country}
                            onChange={(nativePickerValue: string) => setCountry(nativePickerValue)}
                            containerStyle={{ marginTop: 20, width: width * 0.3 }}
                            renderPicker={() => {
                                return (
                                    <View row centerV width={width * 0.2}>
                                        <Image marginR-8 source={
                                            country === 'VND'
                                                ? Assets.images.vietnam
                                                : Assets.images.usa
                                        } />
                                        <Text M16 c04 marginR-9>{country}</Text>
                        ...
                        <TextInput
                            ...
                            style={{ width: width * 0.6, fontFamily: FONTS.bold, fontSize: 30, color: Colors.c04, paddingVertical: 0 }}
                        />
                    </View>
                    <View marginH-small style={{ height: 1, backgroundColor: Colors.cef }} />
                    <View paddingV-small row paddingH-medium spread centerV>
                        <View>
                            <Text M14 c35>1 USD = 22,750 VND</Text>
                            <Text M14 c59>Update on Apr 30, 2018</Text>
                        </View>
                        <ICChange />
                    </View>
                    <View marginH-small style={{ height: 1, backgroundColor: Colors.cef }} />
                    <View row centerV spread paddingT-small paddingH-medium paddingB-40>
                        <Picker
                            useNativePicker
                            value={countryChange}
                            onChange={(nativePickerValue: string) => setCountryChange(nativePickerValue)}
                            containerStyle={{ marginTop: 20, width: width * 0.3 }}
                            renderPicker={() => {
                                return (
                                    <View row centerV width={width * 0.2}>
                                        ...
                                        <Text M16 c04 marginR-9>{countryChange}</Text>
                                        <Image source={Assets.icons.ic_arr_down} />
                                    </View>
                                );
                            }}
