...
const DATA = [
    ...
    {
        name: 'Cynthia Lamb',
        comment: 'Nailed it !! Each and every line effortlessly hooked me in. Almost read this trip thrice now. Superb Job !! ',
        time: '5 hours ago',
        reply: '',
        like: '10',
        isLike: true,
    ...
    {
        name: 'Rosa Richards',
        comment: 'Enjoyable read :-) Thank you fir taking all those amazing pictures that make the narrative so much interesting. Good work! ',
        time: '2 hours ago',
        reply: '1 Reply',
        like: '2',
        isLike: false,
...
const TripCommentScreen = () => {
    ...
    const renderChatInput = () => {
        return (
            <View absB width={width} paddingH-medium row centerV height={60} style={styles.wrapper}>
        ...
        <View paddingV-small paddingH-medium backgroundColor={Colors.white} row>
            ...
            <View marginL-small flex>
                <View spread row marginB-8 centerV>
                    <Text B14 c04>{item.name}</Text>
                    <View row center>
                        <Text M12 c35>{item.like} </Text>
                        <Image source={item.isLike ? Assets.icons.ic_like_active : Assets.icons.ic_like} />
                    </View>
                </View>
                <Text M14 c35>{item.comment}</Text>
                <View row>
                    <Text M12 c83 marginR-small>{item.time}</Text>
                    {item.reply !== '' && <Text M12 c83 marginR-small>{item.reply}</Text>}
                    <Text M12 c83>Reply</Text>
                </View>
            </View>
        </View>
    )
