...
const CollectionScreen = () => {
  ...
  const onNavigateTravelActivitiesScreen = () =>
    navigation.navigate(Routes.TravelActivity);
  const onNavigateDetail = (title: string) =>
    navigation.navigate(Routes.TravelActivityDetail, {
      title,
    });
  ...
  const renderOtherTravel = (item: any, idx: number) => (
    <TouchableWithoutFeedback
      key={`item_${idx}`}
      onPress={() => onNavigateDetail(item.title)}>
      <View marginB-small>
